...
class ShapEngine(object):
    def __init__(self, X, y, X_val, y_val, 
                 ...
                 max_iters=100, use_parallel_knn=False, **kwargs):
        """
        Args:
        ...
        self.use_parallel_knn=use_parallel_knn
    ...
    def run(self, loo_run=True, knn_run=False, weights_list=None):
        ...
        if knn_run is True:
            if self.use_parallel_knn:
                self._calculate_knn_parallel(n_neighbors=10)
            else:
    ...
    def _calculate_knn_parallel(self, n_neighbors=10):
        """
        Calculate the KNN-SHAP.
        from https://github.com/AI-secure/Shapley-Study/blob/master/shapley/measures/KNN_Shapley.py
        """
        ...
        def _calculate_knn_parallel_helper(X_val_sample, y_val_sample):
            # X_val_sample, y_val_sample = args
            knn_mat_ = np.zeros(self.n_sources)
            diff=(self.X - X_val_sample).reshape(self.n_sources, -1)
            dist=np.einsum('ij, ij->i', diff, diff)
            idx=np.argsort(dist)
            ans=self.y[idx]
            knn_mat_[idx[self.n_sources-1]] = float(ans[self.n_sources-1]==y_val_sample) / self.n_sources
            
            cur = np.arange(self.n_sources-2, -1, -1).astype(int)
            const_factor = np.minimum(cur+1, n_neighbors)/(cur+1)
            knn_mat_[idx[cur]] = np.cumsum(knn_mat_[idx[cur+1]] + ((ans[cur]==y_val_sample)*1. - (ans[cur+1]==y_val_sample)*1.) / n_neighbors * const_factor)

            return knn_mat_

        results = Parallel(n_jobs=12, prefer='threads')(delayed(_calculate_knn_parallel_helper)(X_val_sample, y_val_sample) for (X_val_sample, y_val_sample) in tzip(self.X_val, self.y_val))
        # results = [_calculate_knn_parallel_helper(X_val_sample, y_val_sample) for (X_val_sample, y_val_sample) in tzip(self.X_val, self.y_val)]
        # with multiprocess.Pool(12) as p:
        #     results = p.map(_calculate_knn_parallel_helper, [(X_val_sample, y_val_sample) for (X_val_sample, y_val_sample) in zip(self.X_val, self.y_val)])

        knn_mat = np.array(results).T
    ...
    def _calculate_marginal_contributions(self, model_name='model_name'):
        ...
        for iters in tqdm(range(100*self.max_iters), desc=f'Compute marginal contributions MC({model_name})'):
    ...
    def _calculate_marginal_contributions_core(self):
        ...
        for _ in tqdm(range(100), desc=f'Compute marginal contributions MC ...'):
